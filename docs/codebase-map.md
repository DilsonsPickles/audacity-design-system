...
| `apps/sandbox/src/components/editor/MacrosDockPanel.tsx` | Sandbox wiring for the dockable `MacrosPanel` — reads `useMacros()`, owns import (file picker → JSON parse with a type guard) / export (JSON blob download) plumbing; "run" is simulated with a toast (no command engine in the sandbox) |
| `apps/sandbox/src/contexts/MacrosContext.tsx` | Macro data + panel state: `macros` array with mutation helpers (`addMacro` seeds the `END` step, `moveStep`/`deleteStep`/`addCommandToMacro`), dockable-panel open/side (`isMacrosPanelOpen`, `macrosPanelSide`), and `editingMacroId` for the floating editor. Provider sits inside `DialogProvider` in `App.tsx`'s `ThemedApp` tree |
