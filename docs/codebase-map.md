...
| `apps/sandbox/src/contexts/MacrosContext.tsx` | Macro data + panel state: `macros` array with mutation helpers (`addMacro` creates an empty macro — no AU3-style `END` terminator; import strips them, the runner skips any that arrive — plus `moveStep`/`deleteStep`/`addCommandToMacro`), panel open/placement (`isMacrosPanelOpen`, `macrosPanelSide`: `'left' | 'right' | 'bottom' | 'floating'`, right dock is the first-open default), and `editingMacroId` for the floating editor. Provider sits inside `DialogProvider` in `App.tsx`'s `ThemedApp` tree |
...
| `packages/components/src/MacroEditorDialog/MacroEditorDialog.tsx` | Floating "Edit macro" dialog — the full authoring surface: header macro actions (Rename via RenameMacroDialog / Delete / Export), a card-style step list (gripper drag-reorder, per-row pencil→EditStepDialog for parameters, trash, kebab Move up/down), New step via SelectCommandDialog, Done footer. All edits apply live (auto-save); deliberately offers no run actions — running lives in the MacrosPanel rows. Opened via `MacrosContext.editingMacroId` from `AppDialogs.tsx` |
| `packages/components/src/CommandParametersDialog/CommandParametersDialog.tsx` | Per-command macro parameter window (checkbox-gated optional rows, full-width required controls like Mode, Cancel/OK). Sibling `macroParams.ts` holds `parseMacroParameters`/`serializeMacroParameters` for the `Key="value", ...` string format. Opened by MacroEditorDialog's pencil when its `getCommandParameters` prop returns a schema |
