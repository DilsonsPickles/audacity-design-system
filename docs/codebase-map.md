...
**Clip overlap / crossfades (2026-09-21)** — clips may overlap; array position is the z-order (moves raise). Crossfade geometry (edge-overlap regions + equal-power curves, drawn as the X by `TrackNew`) is `packages/components/src/utils/clipCrossfades.ts`; the audible mirror (gain segments baked into playback/mixdown buffers, incl. containment occlusion-mute) is `packages/audio/src/crossfadeGain.ts`. The two MUST stay in agreement — see the product rule in CLAUDE.md. The old `resolveOverlap.ts` eviction machinery is deleted.
