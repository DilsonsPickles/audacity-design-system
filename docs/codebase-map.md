...
| `apps/sandbox/src/data/commandParameters.ts` | Parameter schemas per command: hand-authored Audacity-flavored schemas for ~35 common commands, a deterministic name-seeded mock generator for the other ~330 (themed pools by name heuristics; AU plugins get a Preset selector), and `getDefaultParameters` used to seed new steps |
